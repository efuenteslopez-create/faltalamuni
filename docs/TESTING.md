...
| Verificación y crédito (iteración 1) | `lib/services/__tests__/verification-hardening.test.ts` (19 tests) | Hallazgo 1: admin/moderador/funcionario/agencia no verifican directo ni votan (403 vía `mapError`); hallazgo 2: quórum vía A (autor + vecino verificado), vía B (3 verificados), 3 no verificados no resuelven, cuenta vinculada a la muni no vota, auditoría `report.verification_resolved` con vía y votos, concurrencia sin doble resolución; hallazgo 3: reconocer/responder/derivar no dan crédito, acción acreditable + verificado → "Ya estuvo la Muni", verificado sin gestión → "Problema resuelto", agencia externa no registra acciones |
